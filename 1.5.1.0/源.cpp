...
void LoadPlayList()
{
    ...
    if(!PathFileExists(g_ConfigPath))
    {
        ...
        g_Battery = true;
        WritePrivateProfileString(L"Setting", L"Loop", L"1", g_ConfigPath);
        g_Loop = true;
    }
    else
    {
        ...
        temp = GetPrivateProfileInt(L"Setting", L"Loop", -1, g_ConfigPath);
        if(temp == -1)
        {
            WritePrivateProfileString(L"Setting", L"Loop", L"1", g_ConfigPath);
            g_Loop = true;
        }
        else g_Loop = (temp == 0 ? false : true);
    }
    ...
    switch(msg)
    {
        ...
        case WM_TRAY:
        {
            if(lParam == WM_RBUTTONUP)
            {
                ...
                if(g_Loop)AppendMenu(g_Menu, MF_STRING, ID_Menu_Loop, L"视频循环播放：是");
                else AppendMenu(g_Menu, MF_STRING, ID_Menu_Loop, L"视频循环播放：否");
                wchar_t buf[22];
                _snwprintf(buf, ARRAYSIZE(buf), L"独立息屏控制 滚轮修改时间:%d秒", g_Timeout);
                ...
                if(g_Screenoff)
                {
                    ...
                    AppendMenu(g_Menu, MF_STRING, ID_Menu_TimeoutFirst, L"├─ 1.到点即关");
                    AppendMenu(g_Menu, MF_STRING, ID_Menu_PlayThenTimeout, L"├─ 2.至少放完再看点");
                    AppendMenu(g_Menu, MF_STRING, ID_Menu_PlayFirst, L"└─ 3.放完即关");
        ...
        case WM_COMMAND:
        {
            switch(LOWORD(wParam))
            {
                ...
                case ID_Menu_Loop:
                    g_Loop = !g_Loop;
                    WritePrivateProfileString(L"Setting", L"Loop", g_Loop ? L"1" : L"0", g_ConfigPath);
                    break;
...
LRESULT CALLBACK AboutProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(msg)
    {
        ...
        case WM_PAINT:
        {
            ...
            DrawText(hdc, TEXT("版本：1.5.1.0                日期：2026.3.24"), -1, &RectDrawText, DT_LEFT | DT_SINGLELINE);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)
{
    ...
    RegisterClassEx(&wca);
    g_WindowSize = AdjustClientSize(520, 250, true, &g_ClientSize, WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_VISIBLE, false, NULL);
