...
## 使用方法：
需从Releases下载主程序和libVLC库，解压后放在一起：
LockEngine
│  Config.ini（此文件在首次运行时会自动生成）
│  libvlc.dll（来自libVLC压缩包）
│  libvlccore.dll（来自libVLC压缩包）
│  LockEngine.exe
│
├─PlayList（此文件夹在首次运行时会自动生成）
│  在这里放置你中意的视频壁纸文件，支持mp4,avi,mkv,mov,ts,flv格式
└─plugins（来自libVLC压缩包）
