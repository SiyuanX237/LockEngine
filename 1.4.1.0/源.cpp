    ...
    switch(msg)
    {
        ...
        case WM_TRAY:
        {
            if(lParam == WM_RBUTTONUP)
            {
                ...
                if(g_Mute) AppendMenu(hPopupMenu, MF_STRING, ID_Menu_Mute, L"静音：是");
                else AppendMenu(hPopupMenu, MF_STRING, ID_Menu_Mute, L"静音：否");

                if(g_AutoScale)AppendMenu(hPopupMenu, MF_STRING, ID_Menu_AutoScale, L"去除黑边：是");
                else AppendMenu(hPopupMenu, MF_STRING, ID_Menu_AutoScale, L"去除黑边：否");

                if(g_Battery)AppendMenu(hPopupMenu, MF_STRING, ID_Menu_Battery, L"使用电池时允许播放：是");
                else AppendMenu(hPopupMenu, MF_STRING, ID_Menu_Battery, L"使用电池时允许播放：否");
                ...
                if(g_Screenoff)
                {
                    ...
                    AppendMenu(hPopupMenu, MF_STRING, ID_Menu_PlayThenTimeout, L"    3.先播放再超时");
...
LRESULT CALLBACK AboutProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(msg)
    {
        ...
        case WM_PAINT:
        {
            ...
            DrawText(hdc, TEXT("版本：1.4.1.0                日期：2025.11.13"), -1, &RectDrawText, DT_LEFT | DT_SINGLELINE);
