...
void PlayMedia()
{
	...
	if(Screenoff && TimeoutType != ID_Menu_PlayFirst)
    {
        g_LastTime = chrono::steady_clock::now();
...
void LoadPlayList()
{
    ...
    if(!PathFileExists(ConfigPath))
    {
        ...
        WritePrivateProfileString(L"Setting", L"TimeoutType", ch, ConfigPath);
        WritePrivateProfileString(L"Setting", L"Timeout", L"60", ConfigPath);
        WritePrivateProfileString(L"Setting", L"HideTray", L"0", ConfigPath);

    }
    else
    {
        int temp;
        temp = GetPrivateProfileInt(L"Setting", L"Enable", -1, ConfigPath);
        if(temp == -1)
        {
            WritePrivateProfileString(L"Setting", L"Enable", L"1", ConfigPath);
            Enable = true;
        ...
        temp = GetPrivateProfileInt(L"Setting", L"Mute", -1, ConfigPath);
        if(temp == -1)
        {
            WritePrivateProfileString(L"Setting", L"Mute", L"1", ConfigPath);
            Mute = true;
        ...
        temp = GetPrivateProfileInt(L"Setting", L"AutoScale", -1, ConfigPath);
        if(temp == -1)
        {
            WritePrivateProfileString(L"Setting", L"AutoScale", L"1", ConfigPath);
            AutoScale = true;
        ...
        temp = GetPrivateProfileInt(L"Setting", L"Screenoff", -1, ConfigPath);
        if(temp == -1)
        {
            WritePrivateProfileString(L"Setting", L"Screenoff", L"0", ConfigPath);
            Screenoff = false;
        }
        else Screenoff = (temp == 0 ? false : true);

        
        temp = GetPrivateProfileInt(L"Setting", L"TimeoutType", -1, ConfigPath);
        if(temp == -1)
        {
            wchar_t ch[3];
            wsprintf(ch, L"%d", ID_Menu_TimeoutFirst);
        ...
        temp = GetPrivateProfileInt(L"Setting", L"Timeout", -1, ConfigPath);
        if(temp < 1)
        {
            WritePrivateProfileString(L"Setting", L"Timeout", L"60", ConfigPath);
            Timeout = 60;
        ...
        temp = GetPrivateProfileInt(L"Setting", L"HideTray", -1, ConfigPath);
        if(temp == -1)
        {
            WritePrivateProfileString(L"Setting", L"HideTray", L"0", ConfigPath);
            HideTray = false;
    ...
    switch(msg)
    {
        ...
        case WM_COMMAND:
        {
            switch(LOWORD(wParam))
            {
                ...
                case ID_Menu_HideTray:
                {
                    ...
                    {
                        WritePrivateProfileString(L"Setting", L"HideTray", L"1", ConfigPath);
        ...
        case WM_TIMER:
        {
            if(LogonStatus)//在普通桌面?
            {
                ...
                else if(Screenoff && TimeoutType == ID_Menu_TimeoutFirst)//超时优先
                {
                    auto now = chrono::steady_clock::now();
